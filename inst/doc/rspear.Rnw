...
rspear is currently under development and not available on \href{http://cran.r-project.org/}{CRAN} yet. To use rspear you have to install it from github:
<<eval=FALSE>>=
...
\begin{description}[0.5cm]
  ...
  \item[taxa] character; name of column in x, which holdes the taxon-names.
  \item[abundance] character; name of column in x, which holdes the abundances.
...
Generally the defaults are appropiate and one must only specify \textbf{x, taxa, abundance and group}.\\[1em]



\subsubsection{Trait-database}
In order to minimize traffic on server trait-data is saved locally. 

\lstinline|spear()| checks if there is file 'traits.csv' in the working directory and otherwise downloads the trait-data from the web-server \url{http://www.systemecology.eu/spear/spear-calculator/} to a file 'traits.csv' into the working directory. 
...
If spear cannot find an appropiate match in the trait table, the SPEAR-value for this species is set to '0' and a warning printed:
\begin{lstlisting}
  Warning message:
...
\begin{description}[0.5cm]
   ...
   \item[traits] a data.frame with the following columns:
    ...
    \item[match\_val] goodnes of match. '-1' indicates a direct match, 'NA' indicates a failed match. Values betweenn 0 and 0.5 indicate an approximate match (smaller values - better match)
    \end{description}
...
\noindent Decription of the dataset:
\begin{description}[0.5cm]
...
Looking at \lstinline|match_val| we see that there has been no match (match\_val == NA) for taxon 'xxxxxxxxx'  and 'Baetis rodani' has been matched approximately  (match\_val = 0.1) with 'Beatis rhodani'. All other taxa have been matched directly (match\_val = -1). The SPEAR-value for 'xxxxxxxx' has been set to '0'. We could check and clean our input data, but the matches are appropiate in this case.\\[1em]
...
First we make a copy of the returned trait-table.
<<>>=
traits_modi <- sp$traits
head(traits_modi)
@

Then we can modify this table. 

For example we set that \emph{Baetis rhodani} is exposed to pesticides:
<<>>=
traits_modi[traits_modi$taxa_matched %in% "Baetis rhodani", ]
traits_modi[traits_modi$taxa_matched %in% "Baetis rhodani", "exposed"] <- c(1,1)
head(traits_modi)
@

Note that when selecting \emph{Baetis rhodani} from the trait-table, we have to change two values, because of the spelling error in the data.
